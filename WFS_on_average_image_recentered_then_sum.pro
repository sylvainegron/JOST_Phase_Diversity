FUNCTION WFS_on_average_image_recentered_them_sum, ConfigFiles,dirdatas, file_bg_Focuss,file_bg_Defocuss,file_image_Focuss,file_image_Defocuss,defoc_array_microns,Correction_myopes

  ;Calculate focus and defocus images by recentering all the images and then summing them
  ;Apply WFS on it
  ;return zernikes
  ...
  ;nombre de pixels NP
  NP=ConfigFile[1]
  ;nombre de modes NM (starts with tip, tilt)
  NM=ConfigFile[4]
  ;Surech
  surech = 1.47
...
im=float(im)
for i=1, Nb_images do begin
;SUBSTRACT IMAGES BY AVERAGE BG
im[*,*,i] = CCD_orientation_correction(float(readfits(dirdata+file_image_Focus+float2string(i)+'.fit')))-bg_average_Focus
;RECENTER IMAGE
im[*,*,i]=recenter(im[*,*,i])
endfor
...
im_defoc=fltarr(NP,NP,Nb_images+1)
im_defoc=float(im_defoc)
for i=1, Nb_images do begin
;SUBSTRACT IMAGES BY AVERAGE BG
im_defoc[*,*,i] = CCD_orientation_correction(float(readfits(dirdata+file_image_Defocus+float2string(i)+'.fit')))-bg_average_Defocus
;RECENTER IMAGE
im_defoc[*,*,i]=recenter(im_defoc[*,*,i])
endfor
;average image
im_average_defoc = im_defoc[*,*,0]
for i=1, Nb_images do im_average_defoc=im_average_defoc+im[*,*,i]
im_average_defoc=im_average_defoc/Nb_images


images = [[[im_average]],[[im_average_defoc]]]

writefits, dirdata+'im'+file_image_Focus+'.fit',im_average
writefits, dirdata+'im_defoc'+file_image_Defocus+'.fit',im_average_defoc
